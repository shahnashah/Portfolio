...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const [, setCurrentIndex] = useState(0); // Current index is tracked
  ...
  const scrollLeft = () => {
    if (carouselRef.current) {
      ...
      setCurrentIndex((prev) => Math.max(prev - 1, 0)); // Decrement index
    }
  };
  ...
  const scrollRight = () => {
    if (carouselRef.current) {
      carouselRef.current.scrollBy({ left: 300, behavior: "smooth" });
      setCurrentIndex((prev) => Math.min(prev + 1, items.length - 1)); // Increment index
    }
  };
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      ...
      setCurrentIndex(index); // Update current index on card close
    }
  };
...
// Card component remains unchanged, as it interacts with CarouselContext and uses currentIndex accordingly.
export const Card = ({
  card,
  index,
  layout = false,
